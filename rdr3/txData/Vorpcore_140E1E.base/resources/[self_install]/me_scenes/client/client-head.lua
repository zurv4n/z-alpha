...
Citizen.CreateThread(function()
    TriggerServerEvent("darkk_ops_top:getscenes")
    while true do
        Citizen.Wait(1)
...
RegisterCommand('scenetop', function(source, args, raw)
    TriggerServerEvent("darkk_ops_top:delete")
    TriggerEvent("darkk_ops_top:start")
 end)

RegisterCommand('cscenetop', function(source, args, raw)
    TriggerServerEvent("darkk_ops_top:delete")
 end)

RegisterNetEvent('darkk_ops_top:sendscenes')
AddEventHandler('darkk_ops_top:sendscenes', function(scenes)
    Scenes = scenes
end)

RegisterNetEvent('darkk_ops_top:start')
AddEventHandler('darkk_ops_top:start', function()
    local scenetext = ""
    Citizen.CreateThread(function()
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            TriggerServerEvent("darkk_ops_top:add", scenetext)
